...
# Go back one line to preserve the closing div of the previous block
end_idx = content.rfind('</div>', start_idx, end_idx) + 6 + 1
...
if start_idx != -1 and end_idx != -1:
    # Just replace from start_idx to the parent div's end
    
    # Actually, it's safer to just split and combine
    # Let's find exactly the part to replace
    # We will replace from start_marker until the end of the <div class="relative md:col-span-2"> that contains it and the next one (Caracteres)
    
    # It's easier: just replace everything between '<!-- Proyecto -->' and '<!-- CARD 2: UBICACIÓN FÍSICA -->' except we need to leave the end of the card container intact.
    
    # Let's just find "<!-- CARD 2: UBICACIÓN FÍSICA -->"
    end_idx = content.find('<!-- CARD 2: UBICACIÓN FÍSICA -->', start_idx)
    # the end of the previous card is the `</div>` just before it.
    end_idx = content.rfind('</div>', start_idx, end_idx)
    end_idx = content.rfind('</div>', start_idx, end_idx) # wait, there's `</div> </div>` at the end of card 1.
    pass

# I'll just use regex but with correct markers
import re
match = re.search(r'(<!-- Proyecto -->.*?)(\s*</div>\s*</div>\s*<!-- CARD 2: UBICACIÓN FÍSICA -->)', content, re.DOTALL)
if match:
