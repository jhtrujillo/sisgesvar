      ...
      <div v-else class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-4 scrollbar-custom bg-slate-100/50">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-10">
            <div v-for="cell in sortedMales" :key="cell.varB" :class="['rounded-xl p-4 transition-all relative overflow-hidden', getCardStyle(cell).container]">
              <div class="flex justify-between items-start mb-3 mt-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    ...
                    class="flex items-center justify-center space-x-1 rounded-lg border px-2 py-1 shadow-sm text-[10px] transition-colors"
                    :class="
                      cell.viabilidad
              ...
              <div class="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden" title="Índice Combinado">
                <div class="h-full transition-all duration-500" :class="getAffinityColor(cell)" :style="{ width: getAffinity(cell) + '%' }"></div>
              </div>
            </div>
...
const getCardStyle = (cell: any) => {
  ...
  const baseBorder = isSelected ? "ring-2 shadow-md border-[3px] " : "opacity-70 hover:opacity-100 hover:-translate-y-0.5 hover:shadow-md border-2 ";
