  ...
  <div>
    ...
    <div class="flex items-center space-x-2">
      ...
      <button 
        type="button" 
        @click="openModal" 
        class="px-4 py-2 bg-slate-100 hover:bg-teal-50 text-slate-700 hover:text-teal-700 border border-slate-300 rounded-lg shadow-sm text-sm font-semibold transition-colors flex-shrink-0 flex items-center"
      >
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div class="px-6 py-3 bg-white border-b border-slate-100 flex items-center space-x-2 overflow-x-auto">
          <button 
            ...
            :disabled="!parentPath"
            class="p-1.5 rounded-md hover:bg-slate-100 text-slate-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        ...
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
          <div class="flex-1 mr-4">
            ...
            <span v-else class="text-xs text-slate-400">
              Navegue y seleccione un archivo
            </span>
          </div>
          <div class="flex space-x-3">
            <button @click="closeModal" class="px-5 py-2 rounded-xl text-slate-600 hover:bg-slate-200 font-semibold text-sm transition-colors">
              Cancelar
            </button>
            <button 
              @click="confirmSelection" 
              :disabled="!selectedPath || selectedIsDirectory"
              class="px-5 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold text-sm shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
