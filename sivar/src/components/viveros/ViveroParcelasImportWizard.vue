            ...
            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
              <div class="flex justify-between items-center mb-4 border-b pb-3">
                <h3 class="text-xl leading-6 font-bold text-slate-800" id="modal-title">
                  Asistente de Importación de Parcelas
                </h3>
              ...
              <div v-if="step === 1" class="py-4">
                <div class="border-2 border-dashed border-slate-300 rounded-xl p-12 text-center hover:bg-slate-50 transition-colors cursor-pointer group" @click="$refs.fileInput.click()">
                  <svg class="mx-auto h-16 w-16 text-slate-400 group-hover:text-cenicana transition-colors" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
              ...
              <div v-if="step === 2" class="py-4">
                ...
                <select v-model="selectedSheet" class="mt-1 block w-full pl-3 pr-10 py-2.5 text-base border-slate-300 focus:outline-none focus:ring-1 focus:ring-cenicana focus:border-cenicana rounded-lg border shadow-sm">
                  <option v-for="sheet in sheets" :key="sheet" :value="sheet">{{ sheet }}</option>
                </select>
                <div class="mt-8 flex justify-end">
                  <button @click="processSheet" :disabled="!selectedSheet" class="bg-cenicana text-white font-bold px-6 py-2.5 rounded-lg shadow-sm hover:bg-cenicana-800 disabled:opacity-50 transition-colors">Siguiente</button>
                </div>
              </div>
              ...
              <div v-if="step === 3" class="py-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                    ...
                    <select v-model="mapping.plot" class="w-full pl-3 pr-10 py-2 border-slate-300 rounded-md border text-sm focus:outline-none focus:ring-1 focus:ring-cenicana bg-slate-50">
                  ...
                  <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                    ...
                    <select v-model="mapping.plot_origen" class="w-full pl-3 pr-10 py-2 border-slate-300 rounded-md border text-sm focus:outline-none focus:ring-1 focus:ring-cenicana bg-slate-50">
                      <option value="">-- Opcional --</option>
                      <option v-for="col in headers" :key="col" :value="col">{{ col }}</option>
                    </select>
                    <p class="text-[10px] text-slate-500 mt-2 font-mono bg-slate-100 p-1 rounded">El ID Plot se autocalculará como: {{ viveroIdentificador }}-XX</p>
                  </div>
                  <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm border-l-4 border-l-cenicana">
                    ...
                    <select v-model="mapping.variedad" class="w-full pl-3 pr-10 py-2 border-slate-300 rounded-md border text-sm focus:outline-none focus:ring-1 focus:ring-cenicana bg-slate-50">
                      <option value="">-- Seleccionar --</option>
                      <option v-for="col in headers" :key="col" :value="col">{{ col }}</option>
                    </select>
                  </div>
                ...
                <div class="mt-8 flex justify-between">
                  <button @click="step = sheets.length > 1 ? 2 : 1" class="text-slate-600 px-4 py-2 rounded-lg hover:bg-slate-100 font-medium transition-colors">Volver</button>
                  <button @click="validateData" :disabled="!mapping.plot || !mapping.variedad || isAnalyzing" class="bg-cenicana text-white font-bold px-6 py-2.5 rounded-lg shadow hover:bg-cenicana-800 disabled:opacity-50 transition-colors flex items-center gap-2">
              ...
              <div v-if="step === 4" class="py-2">
                <div v-if="conflicts.length === 0">
                  <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-6 mb-4 text-center">
                    ...
                    <p class="text-sm text-emerald-600 mt-1">Las {{ readyToImport.length }} variedades encontradas coinciden perfectamente con nuestra base de datos.</p>
                  </div>
                </div>
                <div v-else>
                  <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
                    <div class="flex items-start">
                      <svg class="h-5 w-5 text-amber-500 mt-0.5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>
                      <div class="ml-3">
                        <h4 class="text-sm font-bold text-amber-800">Atención requerida</h4>
                        <p class="text-sm text-amber-700 mt-1">Hay <strong>{{ unresolvedCount }}</strong> variedades (de {{ conflicts.length }} conflictos) del Excel que no encontramos en SIVAR. Por favor, asígnales una correcta. <br><strong>Nota:</strong> Las parcelas que dejes sin resolver (en rojo) serán omitidas y NO se importarán.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="max-h-96 overflow-y-auto border border-slate-200 rounded-lg shadow-inner bg-slate-50 relative">
                    <div v-if="displayedConflicts.length === 0" class="p-8 text-center text-slate-500">
                    ...
                    <table v-else class="min-w-full divide-y divide-slate-200 text-sm">
                      ...
                      <tbody class="bg-white divide-y divide-slate-100">
                        <tr v-for="(conflict, index) in displayedConflicts" :key="index + '-' + conflict.excelVariedad" :class="{'bg-emerald-50/50': conflict.resolvedId}">
                          ...
                          <td class="px-4 py-3 relative">
                            <div v-if="!conflict.resolvedId">
                              ...
                              />
                              <div v-if="conflict.showDropdown" class="absolute z-20 w-[90%] mt-1 bg-white shadow-xl max-h-48 rounded-lg py-1 text-xs overflow-auto border border-slate-200 left-4">
                            ...
                            <div v-else class="flex items-center justify-between bg-white border border-emerald-200 rounded-lg px-3 py-1.5">
                              ...
                              <button @click="conflict.resolvedId = null" class="text-xs text-slate-400 hover:text-red-500 underline">Cambiar</button>
                            </div>
                          </td>
                          <td class="px-2 py-3 text-center">
                            <button @click="removeConflict(conflict)" title="Omitir fila" class="text-slate-400 hover:text-red-500 transition-colors p-1.5 rounded hover:bg-red-50">
                ...
                <div class="mt-8 flex justify-between items-center">
                  <button @click="step = 3" class="text-slate-600 px-4 py-2 rounded-lg hover:bg-slate-100 font-medium transition-colors">Volver</button>
                  <div class="flex items-center gap-3">
                    ...
                    <button @click="submitImport" :disabled="!allConflictsResolved || isSubmitting" class="bg-cenicana text-white font-bold px-6 py-2.5 rounded-lg shadow-sm hover:bg-cenicana-800 disabled:opacity-50 flex items-center gap-2 transition-colors">
...
const hideConflictDropdown = (conflict: any) => {
  setTimeout(() => { conflict.showDropdown = false; }, 200);
};
