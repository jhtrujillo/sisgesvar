    ...
    <div class="w-full md:w-1/4 bg-white rounded-xl border border-slate-200 flex flex-col overflow-hidden shadow-sm">
      <div class="p-4 bg-slate-50 border-b border-slate-200 flex flex-col gap-3 sticky top-0 z-10">
        <div class="flex justify-between items-center font-bold text-slate-700">
          ...
          <span class="text-xs font-normal text-slate-500 bg-white px-2 py-1 rounded-md border shadow-sm"
            >{{ filteredFemales.length }} / {{ females.length }}</span
          >
        </div>
        <div class="relative">
          ...
          />
          <div
            v-if="showFemaleDropdown && filteredFemales.length > 0"
            class="absolute z-50 mt-1 w-full bg-white rounded-md shadow-lg border border-slate-200 max-h-48 overflow-y-auto"
          >
      ...
      <div v-else class="flex-1 flex flex-col overflow-hidden">
        <div class="p-4 bg-slate-50 border-b border-slate-200 flex flex-wrap justify-between items-center gap-2">
          ...
          <div class="flex items-center gap-2">
            <input
              v-model="searchMaleQuery"
              type="text"
              placeholder="Filtrar padres..."
          ...
          <table class="w-full text-xs border-collapse">
              ...
              <tr class="bg-slate-100 border-b border-slate-200 text-slate-700">
                <th class="px-3 py-2.5 text-left font-bold cursor-pointer hover:bg-slate-200 select-none" @click="setSort('varB')">
                  <div class="flex items-center gap-1">
                    ...
                    <svg
                      ...
                      :class="sortCol === 'varB' ? 'text-emerald-600' : 'text-slate-400'"
                      viewBox="0 0 16 16"
                      fill="currentColor"
                    >
                      <path d="M8 11L4 7h8z" />
                ...
                <th class="px-3 py-2.5 text-center font-bold cursor-pointer hover:bg-slate-200 select-none w-20" @click="setSort('dg')">
                  <div class="flex items-center justify-center gap-1">
                    DG
                    <svg class="w-3 h-3 shrink-0" :class="sortCol === 'dg' ? 'text-emerald-600' : 'text-slate-400'" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M8 11L4 7h8z" />
                    </svg>
                  </div>
                </th>
                <th class="px-3 py-2.5 text-center font-bold cursor-pointer hover:bg-slate-200 select-none w-24" @click="setSort('vm')">
                  <div class="flex items-center justify-center gap-1">
                    VM Macho
                    <svg class="w-3 h-3 shrink-0" :class="sortCol === 'vm' ? 'text-emerald-600' : 'text-slate-400'" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M8 11L4 7h8z" />
                    </svg>
                  </div>
                </th>
                <th class="px-3 py-2.5 text-center font-bold cursor-pointer hover:bg-slate-200 select-none w-20" @click="setSort('ic')">
                  <div class="flex items-center justify-center gap-1">
                    Índice
                    <svg class="w-3 h-3 shrink-0" :class="sortCol === 'ic' ? 'text-emerald-600' : 'text-slate-400'" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M8 11L4 7h8z" />
                    </svg>
                  </div>
                </th>
                <th class="px-3 py-2.5 text-center font-bold cursor-pointer hover:bg-slate-200 select-none w-16" @click="setSort('polen')">
                  <div class="flex items-center justify-center gap-1">
                    Polen
                    <svg class="w-3 h-3 shrink-0" :class="sortCol === 'polen' ? 'text-emerald-600' : 'text-slate-400'" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M8 11L4 7h8z" />
                    </svg>
                  </div>
                </th>
                <th class="px-3 py-2.5 text-center font-bold cursor-pointer hover:bg-slate-200 select-none w-16" @click="setSort('disp')">
                  <div class="flex items-center justify-center gap-1">
                    Disp.
                    <svg class="w-3 h-3 shrink-0" :class="sortCol === 'disp' ? 'text-emerald-600' : 'text-slate-400'" viewBox="0 0 16 16" fill="currentColor">
                      <path d="M8 11L4 7h8z" />
                    </svg>
                  </div>
                </th>
            ...
            <tbody>
              <template v-for="(cell, idx) in paginatedMales" :key="cell.varB">
                <tr
                  @click="toggleDetail(cell)"
                  :class="[
                    'border-b border-black/5 cursor-pointer transition-all duration-150',
                    getRowBg(cell),
                    expandedCell === cell ? 'outline outline-2 outline-emerald-400' : 'hover:brightness-95'
                  ]"
                >
                  <td class="px-3 py-2.5 font-bold" :class="getRowTextMain(cell)">
                    <div class="flex items-center gap-2">
                      <span
                        ...
                        :class="isDarkRow(cell) ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-600'"
                      >
                  ...
                  <td class="px-3 py-2.5 text-center">
                    <div class="flex items-center justify-center gap-1.5">
                      ...
                      <button
                        @click.stop="$emit('toggle-cross', cell)"
                        :class="[
                          'inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold border transition-all',
                          cell.viabilidad
                            ? 'bg-emerald-600 text-white border-emerald-700 shadow hover:bg-emerald-700'
                            : 'bg-white/70 text-slate-600 border-slate-300 hover:bg-white hover:border-emerald-400'
                        ]"
                      >
                  ...
                  <td colspan="7" class="px-4 py-3 border-b-2 border-emerald-400">
                    <div class="bg-white/80 rounded-lg p-3 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs shadow-inner">
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Variedad Padre</span
                        ><span class="font-black text-slate-800 text-base">{{ cell.varB }}</span>
                      </div>
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Dist. Genética</span
                        ><span class="font-black text-slate-800 text-lg">{{ getDistanciaLocal(cell.varA, cell.varB) }}</span>
                      </div>
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">VM Macho</span
                        ><span class="font-black text-slate-800 text-lg">{{ cell.vm2 }}</span>
                      </div>
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Índice Combinado</span
                        ><span class="font-black text-slate-800 text-lg">{{ getIcValue(cell) }}</span>
                      </div>
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Polen</span
                        ><span class="font-black text-slate-800">{{ cell.polen2 }}%</span>
                      </div>
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Disponibilidad</span
                        ><span class="font-black text-slate-800">{{ getDisp(cell.varB) }}</span>
                      </div>
                      <div class="flex flex-col gap-0.5">
                        <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Estado</span
                        ><span :class="cell.viabilidad ? 'text-emerald-700 font-black' : 'text-slate-500 font-semibold'">{{
                          cell.viabilidad ? "✓ Seleccionado" : "No seleccionado"
                        }}</span>
                      </div>
                      ...
                      <div class="flex flex-col gap-0.5">
                        ...
                        >
                          <span>⚙️ Ajustar:</span>
                          <span :class="cell.viabilidad ? 'text-emerald-700' : 'text-slate-400'"
                            >{{ cell.flores_madre ?? 1 }} / {{ cell.flores_padre ?? 1 }}</span
                          >
                        </button>
                      </div>
                      ...
                      <div class="flex items-end">
                        <button
                          @click.stop="$emit('toggle-cross', cell)"
                          class="px-4 py-1.5 rounded-lg font-bold text-xs transition-all w-full"
                          :class="cell.viabilidad ? 'bg-rose-500 text-white hover:bg-rose-600' : 'bg-emerald-600 text-white hover:bg-emerald-700'"
                        >
        ...
        <div class="p-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          <span class="text-xs text-slate-500"
            >Mostrando {{ currentPage * PAGE_SIZE + 1 }}–{{ Math.min((currentPage + 1) * PAGE_SIZE, filteredSortedMales.length) }} de
            {{ filteredSortedMales.length }} padres</span
          >
          <div class="flex items-center gap-1">
            <button
              @click="currentPage = 0"
              :disabled="currentPage === 0"
              class="px-2 py-1 text-xs rounded border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed font-bold"
            >
              «
            </button>
            <button
            ...
            <span class="px-3 py-1 text-xs font-bold text-emerald-700 bg-emerald-50 rounded border border-emerald-200"
...
watch(
  () => props.selectedFemaleRow,
  () => {
    currentPage.value = 0;
    expandedCell.value = null;
  }
);
watch(searchMaleQuery, () => {
  currentPage.value = 0;
});
...
const filteredSortedMales = computed(() => {
  ...
  list.sort((a, b) => {
    ...
    let vA = 0,
      vB = 0;
    if (sortCol.value === "dg") {
      vA = Number(props.getDistanciaLocal(a.varA, a.varB)) || 0;
      vB = Number(props.getDistanciaLocal(b.varA, b.varB)) || 0;
    } else if (sortCol.value === "vm") {
      vA = Number(a.vm2) || 0;
      vB = Number(b.vm2) || 0;
    } else if (sortCol.value === "ic") {
      vA = getIcValue(a);
      vB = getIcValue(b);
    } else if (sortCol.value === "polen") {
      vA = Number(a.polen2) || 0;
      vB = Number(b.polen2) || 0;
    } else if (sortCol.value === "disp") {
      vA = Number(props.getDisp(a.varB)) || 0;
      vB = Number(props.getDisp(b.varB)) || 0;
    }
...
const toggleDetail = (cell: any) => {
  expandedCell.value = expandedCell.value === cell ? null : cell;
};
...
const getRowBg = (cell: any): string => {
  ...
  if (dg >= 0.65) return "bg-blue-600";
  if (dg >= 0.55) return "bg-sky-400";
  if (dg >= 0.45) return "bg-slate-200";
  if (dg >= 0.35) return "bg-amber-300";
...
const getIcBadge = (cell: any): string => {
  const ic = getIcValue(cell);
  if (ic >= 80) return "bg-indigo-100 text-indigo-800";
  if (ic >= 65) return "bg-sky-100 text-sky-800";
  if (ic >= 50) return "bg-emerald-100 text-emerald-800";
