            ...
            <thead class="bg-gray-50">
              ...
              <tr v-if="haveColumnFilters" class="bg-gray-100 border-b border-gray-300">
                <template v-for="(col, index) in columns" :key="'filter_' + index">
                  <th v-if="columnsToShow.includes(col.text)" scope="col" class="px-2 py-1 relative">
                    <Popover v-slot="{ open }" class="relative">
                      <PopoverButton
                        class="flex items-center justify-between w-full rounded border border-gray-200 px-2 py-1 text-xs shadow-sm bg-white hover:bg-gray-50 focus:outline-none"
                      >
                        <span class="truncate text-gray-500 font-normal">
                          {{
                            columnFiltersData[col.keyName] && columnFiltersData[col.keyName].length > 0
                              ? `(${columnFiltersData[col.keyName].length}) Selec...`
                              : "Todos"
                          }}
                        </span>
                      ...
                      <PopoverPanel class="absolute z-50 mt-1 w-48 bg-white border border-gray-200 rounded-md shadow-lg max-h-72 overflow-y-auto">
                        <div class="p-2 space-y-1">
                          <div class="flex items-center pb-2 border-b border-gray-100 mb-2">
                            <span class="text-xs text-blue-600 font-bold cursor-pointer hover:underline" @click="clearColumnFilter(col.keyName)"
                              >Limpiar Filtro</span
                            >
                          </div>
                          ...
                          <div
                            v-for="val in (columnDistinctValues[col.keyName] || []).filter(
                              (v) => !columnSearchQueries[col.keyName] || String(v).toLowerCase().includes(columnSearchQueries[col.keyName].toLowerCase())
                            )"
                            :key="val"
...
const { rows, columns, isOrdened, isDelete, isEditable, otherButtonText, haveButtonAddNew, haveButtonExcel, nameExcel, haveSearch, haveColumnFilters } =
  toRefs(props);
