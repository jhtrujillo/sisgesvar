...
<template>
  <div class="w-full max-w-[98%] mx-auto px-2 sm:px-4 space-y-6 pt-2 pb-12 animate-fade-in">
    ...
    <div class="flex justify-start">
      <router-link
        class="group flex items-center px-4 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl shadow-sm hover:bg-emerald-50 hover:text-cenicana-800 hover:border-emerald-200 transition-all duration-200"
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between border-b border-slate-100 pb-4 gap-4">
      <div>
        <h1 class="text-3xl font-extrabold tracking-tight text-slate-800 bg-gradient-to-r from-cenicana-800 to-emerald-600 bg-clip-text text-transparent flex items-center">
        ...
        <p class="mt-1.5 text-xs font-semibold text-slate-450 ml-10">Consulte y filtre el registro completo de cruces completados en la plataforma de mejoramiento.</p>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-premium overflow-hidden">
      <div class="overflow-x-auto scrollbar-custom">
        <table class="min-w-full divide-y divide-slate-100">
          ...
          <tbody class="bg-white divide-y divide-slate-50">
            ...
            >
              <template v-for="column in tableColumns" :key="column.key">
                <td v-if="columnsToShow.includes(column.key)" class="px-5 py-3 whitespace-nowrap text-xs text-slate-700 group-hover:text-slate-900 font-medium border-slate-100">
                  
                  <!-- ID Badge -->
                  <span v-if="column.key === 'id_crzmnto'" class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold bg-slate-50 border border-slate-200/80 text-slate-500 shadow-sm">
                    #{{ crossing[column.key as keyof typeof crossing] }}
                  </span>
                  
                  <!-- Pedigree Styling -->
                  <span v-else-if="column.key === 'pdgree'" class="font-mono text-[10px] py-1 px-2 bg-slate-50 border border-slate-200/40 rounded-lg text-slate-650 font-semibold shadow-sm inline-block">
                    {{ crossing[column.key as keyof typeof crossing] || 'N/A' }}
                  </span>

                  <!-- Variedad Madre Style -->
                  <span v-else-if="column.key === 'vrdad_mdre'" class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-emerald-50 text-emerald-800 border border-emerald-100 shadow-sm cursor-pointer hover:bg-emerald-100 hover:text-emerald-950 transition-colors" @click="openVarietyProfile(crossing[column.key as keyof typeof crossing])">
                    {{ crossing[column.key as keyof typeof crossing] }}
                  </span>
                  
                  <!-- Padres Style -->
                  <span v-else-if="column.key.startsWith('vrdad_pdre')">
                    <span v-if="crossing[column.key as keyof typeof crossing]" class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-sky-50 text-sky-800 border border-sky-100/80 shadow-sm cursor-pointer hover:bg-sky-100 hover:text-sky-950 transition-colors" @click="openVarietyProfile(crossing[column.key as keyof typeof crossing])">
                      {{ crossing[column.key as keyof typeof crossing] }}
                    </span>
      ...
      <!-- Pagination -->
      <div v-if="CrossingsListsStore.totalPages > 1" class="bg-slate-50/50 px-6 py-4 border-t border-slate-100 flex items-center justify-between flex-wrap gap-2">
        ...
        <div class="text-xs font-bold text-slate-500 bg-white border border-slate-200/80 px-3 py-1 rounded-xl shadow-sm">
          Página <span class="text-emerald-700 font-extrabold">{{ CrossingsListsStore.currentPage }}</span> de <span class="text-slate-800 font-extrabold">{{ CrossingsListsStore.totalPages }}</span>
        </div>
