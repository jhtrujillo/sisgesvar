      ...
      <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
        <div class="flex items-center">
          ...
          <select
            ...
            class="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-cenicana focus:border-transparent"
          >
      ...
      <div class="overflow-x-auto border border-gray-200 rounded-lg">
        <table class="min-w-full bg-white">
          ...
          <tbody class="text-gray-600 text-sm font-light">
            ...
            <tr v-else v-for="vivero in paginatedViveros" :key="vivero.id" class="border-b border-gray-200 hover:bg-gray-100">
              ...
              <td class="py-3 px-6 text-center">
                <div class="flex item-center justify-center">
                  ...
                  <button @click="deleteVivero(vivero.id)" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
        ...
        <form @submit.prevent="submitCosecha">
          <div class="p-6 space-y-4">
            <div class="p-3 bg-emerald-50/50 text-emerald-900 text-xs font-semibold rounded-lg border border-emerald-100/50">
              Registrando cosecha para el vivero <span class="font-bold">{{ viveroSeleccionado?.nombre }}</span> (Corte Actual:
              {{ viveroSeleccionado?.numero_corte }}).
            </div>

            <div>
              <label for="fecha_cosecha" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2"
                >Fecha de Cosecha <span class="text-red-500">*</span></label
              >
              <input
            ...
            <div>
              <label for="nueva_fecha_siembra" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2"
                >Nueva Fecha de Siembra <span class="text-red-500">*</span></label
              >
              <input
        ...
        <div class="p-6">
          <div class="mb-4 flex flex-col gap-2">
            <div class="text-sm text-slate-600">
              Historial de cosechas para el vivero <span class="font-bold text-slate-800" v-html="viveroSeleccionado?.nombre"></span> (<span
                class="font-mono text-xs"
                >{{ getBaseCode(viveroSeleccionado?.identificador_unico) }}</span
              >)
            </div>
            ...
            <table class="w-full text-sm text-left">
              ...
              <tbody>
                <tr
                  ...
                  :key="item.id"
                  :class="index !== historial.length - 1 ? 'border-b border-slate-100' : ''"
