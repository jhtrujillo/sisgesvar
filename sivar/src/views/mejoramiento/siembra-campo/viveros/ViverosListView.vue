                ...
                <tr v-if="expandedViveros[vivero.id]">
                  <td colspan="10" class="bg-slate-50 border-b border-gray-200 p-4">
                    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                      ...
                      <div
                        v-if="!vivero.parcelas || vivero.parcelas.length === 0"
                        class="text-xs text-slate-400 italic py-2 text-center bg-slate-50 rounded-lg"
                      >
                      ...
                      <div v-else class="overflow-x-auto rounded-lg border border-slate-100">
                        <table class="w-full text-left border-collapse text-xs">
                          ...
                          <tbody>
                            <tr
                              v-for="p in [...vivero.parcelas].sort((a, b) => a.numero_parcela - b.numero_parcela)"
                              :key="'list_parc_' + p.id"
                            ...
                            >
                              ...
                              <td class="px-4 py-2.5 text-slate-600">
                                ...
                                <span v-else-if="vivero.caracter?.nombre" class="text-slate-400 italic" title="Heredado del Vivero">{{
                                  vivero.caracter.nombre
                                }}</span>
        ...
        <form @submit.prevent="submitCosecha">
          <div class="p-6 space-y-4">
            <div class="p-3 bg-emerald-50/50 text-emerald-900 text-xs font-semibold rounded-lg border border-emerald-100/50">
              Registrando corte para el vivero <span class="font-bold">{{ viveroSeleccionado?.nombre }}</span> (Corte Actual:
            ...
            <div>
              <label for="fecha_cosecha" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2"
                >Fecha del Corte <span class="text-red-500">*</span></label
              >
              <input
            ...
            <div>
              <label for="ambiente" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2"
                >Uso para la Floración <span class="text-red-500">*</span></label
              >
              <select
