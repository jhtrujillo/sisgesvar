        ...
        <div v-show="activeTab === 'generales'" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="fecha_siembra"
                  >Fecha de Siembra / Corte <span class="text-red-500">*</span></label
                >
                <input
              ...
              <!-- Temporada Floración -->
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="temporada_floracion"
                  >Temporada de cruzamientos</label
                >
                <input
          ...
          <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5">
              ...
              <!-- Lote -->
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="lote_id"
                  >Lote <span class="text-red-500">*</span></label
                >
              ...
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="consecutivo_vivero_id"
                  >Vivero <span class="text-red-500">*</span></label
                >
                <select
                ...
                >
                  ...
                  <option v-for="num in availableViveroNumbers" :key="'v_num_' + num" :value="num">Vivero {{ num }}</option>
                </select>
              </div>
            </div>
            ...
            <div
              class="bg-slate-50/50 rounded-2xl p-4 border border-slate-100 mt-4 space-y-3"
              v-if="isEditing && form.historial_lotes && form.historial_lotes.length > 0"
            >
                    ...
                    <tr v-for="hist in form.historial_lotes" :key="hist.id" class="border-b border-slate-100 last:border-0 hover:bg-slate-50/30">
                      ...
                      <td class="py-2.5 px-4 text-center">
                        <span
                          ...
                          :class="[
                            hist.activo ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-slate-100 text-slate-500 border border-slate-200'
                          ]"
                        >
        ...
        <div v-show="activeTab === 'origen'" class="space-y-6">
          <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
              <!-- Origen Ingenio -->
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="origen_ingenio"
                  >Ingenio <span class="text-red-500">*</span></label
                >
                <select
              ...
              <!-- Origen Año -->
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="origen_anio"
              ...
              <!-- Origen Hacienda -->
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="origen_hacienda"
                  >Hacienda <span class="text-red-500">*</span></label
                >
                <select
              ...
              <!-- Origen Lote -->
              <div>
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5" for="origen_lote"
                  >Lote <span class="text-red-500">*</span></label
                >
                <select
              ...
              <div>
                ...
                >
                  ...
                  <option v-for="v in viverosOrigenOptions" :key="'origen_vivero_' + v.id" :value="v.id">{{ v.nombre }} ({{ v.identificador_unico }})</option>
                </select>
              ...
              <div>
                <div class="flex justify-between items-center mb-1.5">
                  <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider" for="origen_parcela_text"> Parcela (Opcional) </label>
                  <button
                    type="button"
                ...
                <select
                  ...
                  class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-xs font-semibold rounded-xl px-3.5 py-3 focus:bg-white focus:ring-4 focus:ring-cenicana/10 focus:border-cenicana transition-all outline-none shadow-sm"
                  :class="{
                    'bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed shadow-inner': !form.origen_vivero_id || origenParcelasOptions.length === 0
                  }"
                  id="origen_parcela_text"
                >
                  <option value="">Seleccione una Parcela (Opcional)</option>
                  <option v-for="p in origenParcelasOptions" :key="'orig_p_' + p.id" :value="p.numero_parcela">
        ...
        <div class="overflow-x-auto bg-white rounded-lg border border-slate-200 shadow-sm">
          <table class="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              <template v-else>
                <tr
                  v-for="p in paginatedParcelas"
                  :key="p.id"
                  class="border-b border-slate-100 transition-colors"
                  :class="{ 'bg-cenicana/5': editingPlotId === p.id, 'hover:bg-slate-50': editingPlotId !== p.id }"
                >
                  ...
                  <template v-if="editingPlotId === p.id">
                    <td class="px-4 py-3 min-w-[200px]">
                      <div class="relative">
                        ...
                        >
                          <div v-if="filteredEditingVariedades.length === 0" class="p-2 text-slate-500">No se encontraron variedades</div>
                          <div
          ...
          <form @submit.prevent="submitTraslado">
            <div class="p-6 space-y-4">
              <div class="p-3 bg-blue-50 text-blue-900 text-xs font-semibold rounded-lg border border-blue-100">
                Estás trasladando el vivero <strong class="font-black">{{ form.identificador_unico }}</strong> a un nuevo lote físico. El lote actual quedará
                liberado.
              </div>
...
const getParcIdPlotOrigen = (p: any) => {
  ...
  if (raw && raw.trim() !== '' && raw !== currentPlotId) {
    return raw.trim();
  }

  const parcNum = p.numero_parcela_origen || p.numero_parcela;
  ...
  if (form.value.origen_vivero_id && allViverosList.value && allViverosList.value.length > 0) {
    ...
    if (parent && parent.identificador_unico) {
      return parcNum ? `${parent.identificador_unico}-${parcNum}` : parent.identificador_unico;
    }
  }

  if (form.value.origen_parcela && form.value.origen_parcela.trim() !== '') {
    const base = form.value.origen_parcela.trim();
    if (base.split('-').length >= 5) {
      return base;
    }
    return parcNum ? `${base}-${parcNum}` : base;
  }
...
const updateIdPlotOrigen = () => {
  ...
  if (!baseOrigen) {
    baseOrigen = form.value.origen_parcela || "";
  }
  ...
  const parcNum = parcelaForm.value.numero_parcela_origen || parcelaForm.value.numero_parcela;
  parcelaForm.value.id_plot_origen = parcNum ? `${baseOrigen}-${parcNum}` : baseOrigen;
};
...
const filteredEditingVariedades = computed(() => {
  ...
  return variedades.value.filter((v) => v.nm_vrdad.toLowerCase().includes(editingPlotForm.value.variedad_name.toLowerCase()));
});
