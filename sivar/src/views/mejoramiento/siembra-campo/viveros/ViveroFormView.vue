  ...
  <div class="container mx-auto p-6 max-w-3xl">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="mb-4 relative md:col-span-1">
          ...
          <div class="relative">
            ...
            <div v-if="showCaracteres && form.proyecto_id" class="absolute z-10 w-full mt-1 bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto sm:text-sm">
              <div
    ...
    <div v-if="isEditing" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 border-t-4 border-cenicana">
      ...
      <div class="bg-slate-50 p-5 rounded-xl border border-slate-200 mb-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 items-start">
          <div>
            ...
            <input v-model="parcelaForm.numero_parcela" type="number" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-cenicana bg-white shadow-sm" />
          </div>
          <div class="relative">
            ...
            <div v-if="showVariedades" class="absolute z-10 w-full mt-1 bg-white shadow-xl max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto sm:text-sm border border-slate-100">
          ...
          <div>
            ...
            <select v-model="parcelaForm.caracter_id" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-cenicana bg-white shadow-sm">
          ...
          <div>
            ...
            <input v-model="parcelaForm.numero_parcela_origen" @input="updateIdPlotOrigen" type="number" placeholder="No." class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-cenicana bg-white shadow-sm" />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">ID Plot Origen</label>
            <input v-model="parcelaForm.id_plot_origen" type="text" class="w-full border rounded-lg px-3 py-2 text-sm bg-slate-100 text-slate-500 cursor-not-allowed shadow-inner font-mono" readonly />
          </div>
        </div>
        
        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
          <button v-if="parcelas.length > 0" type="button" @click="deleteAllParcelas" class="bg-white border border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 font-bold py-2.5 px-6 rounded-lg text-sm transition-colors shadow-sm flex items-center justify-center gap-2 mr-auto">
          ...
          <button type="button" @click="showImportWizard = true" class="bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold py-2.5 px-6 rounded-lg text-sm transition-colors shadow-sm flex items-center justify-center gap-2">
          ...
          <button type="button" @click="submitParcela" :disabled="isSubmittingParcela || !parcelaForm.variedad_id" class="bg-cenicana hover:bg-cenicana-800 text-white font-bold py-2.5 px-6 rounded-lg text-sm transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 min-w-[180px]">
      ...
      <div class="flex justify-between items-center mb-3 mt-4">
        <h4 class="text-sm font-bold text-slate-700">Parcelas Agregadas <span class="bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full text-xs ml-1">{{ parcelas.length }}</span></h4>
      ...
      <div class="overflow-x-auto bg-white rounded-lg border border-slate-200 shadow-sm">
            ...
            <template v-else>
              <tr v-for="p in paginatedParcelas" :key="p.id" class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-3 text-slate-600 text-xs">
                  ...
                  <span v-else-if="form.caracter_id && getCaracterGlobalNombre()" class="text-slate-400 italic" title="Heredado del Vivero">{{ getCaracterGlobalNombre() }}</span>
                ...
                <td class="px-4 py-3 text-center">
                  <button type="button" @click="deleteParcela(p.id)" class="text-red-400 hover:text-red-600 transition-colors bg-red-50 hover:bg-red-100 p-2 rounded-lg" title="Eliminar Parcela">
        ...
        <div v-if="parcelas.length > 0" class="flex flex-col sm:flex-row items-center justify-between border-t border-slate-200 px-4 py-3 bg-slate-50">
          <div class="flex items-center gap-2 mb-2 sm:mb-0">
            ...
            <select v-model="rowsPerPage" @change="currentPage = 1" class="text-sm border border-slate-300 rounded px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-cenicana">
          ...
          <div class="flex items-center gap-4">
            <span class="text-sm text-slate-600">
              <template v-if="Number(rowsPerPage) === -1">
                1 - {{ filteredParcelas.length }} de {{ filteredParcelas.length }}
              </template>
              <template v-else>
                {{ filteredParcelas.length === 0 ? 0 : (currentPage - 1) * Number(rowsPerPage) + 1 }} - {{ Math.min(currentPage * Number(rowsPerPage), filteredParcelas.length) }} de {{ filteredParcelas.length }}
              </template>
            </span>
            <div class="flex items-center gap-1">
              <button @click="currentPage--" :disabled="currentPage === 1" class="p-1.5 rounded text-slate-500 hover:bg-slate-200 disabled:opacity-50 transition-colors" title="Página Anterior">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
              </button>
              <button @click="currentPage++" :disabled="currentPage >= totalPages" class="p-1.5 rounded text-slate-500 hover:bg-slate-200 disabled:opacity-50 transition-colors" title="Siguiente Página">
...
const filteredVariedades = computed(() => {
  ...
  return variedades.value.filter((v) => 
    v.nm_vrdad.toLowerCase().includes(searchVariedad.value.toLowerCase())
  ).slice(0, 100);
});
...
const hideVariedadesDelay = () => {
  setTimeout(() => { showVariedades.value = false; }, 200);
};
