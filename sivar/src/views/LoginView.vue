...
<template>
  <div class="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden antialiased">
    ...
    <div class="relative z-10 flex flex-col sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <div class="bg-white/80 backdrop-blur-xl border border-white/60 shadow-premium rounded-3xl py-10 px-6 sm:px-12 mx-4 sm:mx-0 transition-all duration-300 hover:shadow-premium-hover">
        <div class="text-center mb-8">
          <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight flex items-center justify-center">
            SIVAR
          </h1>
        ...
        <form class="space-y-6" @submit.prevent="Login()">
          ...
          <div>
            <button
              type="submit"
              class="relative w-full flex justify-center items-center py-3 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-gradient-to-r from-cenicana-800 to-emerald-600 hover:from-emerald-700 hover:to-cenicana focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cenicana transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 shadow-md shadow-emerald-500/20"
              :class="{ 'opacity-80 cursor-not-allowed pointer-events-none': isBusy }"
            >
              <span v-if="!isBusy">Iniciar Sesión</span>
              <span v-else>Ingresando...</span>
              
              <div class="ml-2.5" v-if="isBusy">
    ...
    <div class="relative z-10 py-4 mt-4">
      <p class="text-slate-400 text-center text-xs font-medium">
        Cenicaña &copy; {{ new Date().getFullYear() }}. Todos los derechos reservados.
      </p>
    </div>
  </div>
</template>
