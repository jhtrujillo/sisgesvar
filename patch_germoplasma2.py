...
script_idx = content.find('<script setup lang="ts">')
script_content = content[script_idx:]

new_content = new_template + "\n" + script_content

# Reorder columns
table_cols_match = re.search(r'const tableColumns = \[(.*?)\];', new_content, flags=re.DOTALL)
if table_cols_match:
    ...
    if var_match:
        ...
        new_cols_str = f'\n  {extracted},\n' + cols_str.strip()
        new_content = new_content[:table_cols_match.start(1)] + new_cols_str + new_content[table_cols_match.end(1):]


cols_to_show_match = re.search(r'const columnsToShow = ref\(\[(.*?)\]\);', new_content, flags=re.DOTALL)
if cols_to_show_match:
    ...
    if var_match:
        ...
        new_cols_str = f'\n  {extracted},\n' + cols_str.strip()
        new_content = new_content[:cols_to_show_match.start(1)] + new_cols_str + new_content[cols_to_show_match.end(1):]
